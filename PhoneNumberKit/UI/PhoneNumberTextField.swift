...
/// Custom text field that formats phone numbers
open class PhoneNumberTextField: NSTextField, NSTextFieldDelegate {
    public let phoneNumberKit: PhoneNumberKit

    public lazy var flagButton = NSButton()
...
//            leftViewMode = self.withFlag ? .always : .never
    ...
    #if compiler(>=5.1)
    /// Available on iOS 13 and above just.
    public var countryCodePlaceholderColor: NSColor = {
        if #available(macOS 10.10, *) {
            return .secondaryLabelColor
        } else {
            return NSColor(red: 0, green: 0, blue: 0.0980392, alpha: 0.22)
        }
    }() {
        didSet {
            self.updatePlaceholder()
    ...
    /// Available on iOS 13 and above just.
    public var numberPlaceholderColor: NSColor = {
        if #available(macOS 10.10, *) {
...
//        self.autocorrectionType = .no
    ...
    open func updateFlag() {
        ...
        self.flagButton.title = flag + " "
        self.flagButton.setAccessibilityLabel(
            NSLocalizedString(
                "PhoneNumberKit.CountryCodePickerEntryButton.AccessibilityLabel",
                value: "Select your country code",
                comment: "Accessibility Label for Country Code Picker button"
        ...
        if let countryName = Locale.autoupdatingCurrent.localizedString(forRegionCode: self.currentRegion) {
            let selectedFormat = NSLocalizedString(
                ...
                value: "%@ selected",
                comment: "Accessiblity hint for currently selected country code")
            self.flagButton.setAccessibilityHelp(String(format: selectedFormat, countryName))
        }

        let fontSize: CGFloat
        if #available(macOS 11.0, *) {
            fontSize = (font ?? NSFont.preferredFont(forTextStyle: .body)).pointSize
        } else {
            fontSize = font?.pointSize ?? 13
        }
        self.flagButton.font = NSFont.systemFont(ofSize: fontSize)
    }

    open func updatePlaceholder() {
        guard self.withExamplePlaceholder else { return }
//        if isEditing, !self.stringValue.isEmpty { return } // No need to update a placeholder while the placeholder isn't showing
        if !self.stringValue.isEmpty { return } // No need to update a placeholder while the placeholder isn't showing
        ...
        let font: NSFont
        if #available(macOS 11.0, *) {
            font = self.font ?? NSFont.preferredFont(forTextStyle: .body)
        } else {
...
//    }
    ...
    /// containingViewController looks at the responder chain to find the view controller nearest to itself
    var containingViewController: NSViewController? {
        var responder: NSResponder? = self
        while !(responder is NSViewController) && responder != nil {
    ...
    internal func extractCursorPosition() -> CursorPosition? {
        ...
        guard let text = text, let selectedTextRange = currentEditor()?.selectedRange else {
            return nil
        }
        let textAsNSString = text as NSString
        let cursorEnd = selectedTextRange.upperBound
//        let cursorEnd = offset(from: beginningOfDocument, to: selectedTextRange.end)
        // Look for the next valid number after the cursor, when found return a CursorPosition struct
        for i in cursorEnd..<textAsNSString.length {
            let cursorRange = NSRange(location: i, length: 1)
    ...
    // Finds position of previous cursor in new formatted text
    internal func selectionRangeForNumberReplacement(textField: NSTextField, formattedText: String) -> NSRange? {
        let textAsNSString = formattedText as NSString
        var countFromEnd = 0
        guard let cursorPosition = extractCursorPosition() else {
            return nil
...
//        }
    ...
    public func controlTextDidBeginEditing(_ obj: Notification) {
        ...
        self._delegate?.controlTextDidBeginEditing?(obj)
    }
    ...
    public func controlTextDidEndEditing(_ obj: Notification) {
        updateTextFieldDidEndEditing(obj)
        self._delegate?.controlTextDidEndEditing?(obj)
    }

    open override func textDidEndEditing(_ notification: Notification) {
        updateTextFieldDidEndEditing(notification)
        super.textDidEndEditing(notification)
    }

//    @available (iOS 10.0, tvOS 10.0, *)
//    open func textFieldDidEndEditing(_ textField: NSTextField, reason: NSTextField.DidEndEditingReason) {
//        updateTextFieldDidEndEditing(textField)
//        if let _delegate = _delegate {
//            if (_delegate.responds(to: #selector(textFieldDidEndEditing(_:reason:)))) {
//                _delegate.textFieldDidEndEditing?(textField, reason: reason)
...
//    }
//
//    open func textFieldShouldClear(_ textField: NSTextField) -> Bool {
//        return self._delegate?.textFieldShouldClear?(textField) ?? true
//    }
//
//    open func textFieldShouldReturn(_ textField: NSTextField) -> Bool {
//        return self._delegate?.textFieldShouldReturn?(textField) ?? true
//    }

    private func updateTextFieldDidEndEditing(_ notification: Notification) {
        if let textField = notification.object as? NSTextField, // TODO: Check this!
           self.withExamplePlaceholder, self.withPrefix, let countryCode = phoneNumberKit.countryCode(for: currentRegion)?.description,
           textField.stringValue == internationalPrefix(for: countryCode) {
